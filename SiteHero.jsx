// FPI Security — Hero Section (responsive)
const SiteHero = ({ onNavigate, direction = 'command' }) => {
  const isCommand = direction === 'command';
  if (isCommand) return <HeroCommand onNavigate={onNavigate} />;
  return <HeroClarity onNavigate={onNavigate} />;
};

/* ─── Direction A: Command ─────────────────────────────── */
const HeroCommand = ({ onNavigate }) => {
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#E8EDF4', borderRadius: 20, padding: '5px 14px', marginBottom: 22 }}>
            <i data-lucide="map-pin" style={{ width: 12, height: 12, color: '#006090' }}></i>
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            {['Licensed & Insured', 'Armed & Unarmed', '24/7 Operations'].map(b => (
              <div key={b} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <i data-lucide="check-circle" style={{ width: 14, height: 14, color: '#006090' }}></i>
